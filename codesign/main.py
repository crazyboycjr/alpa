...
def add_args(parser):
    ...
    parser.add_argument(
        ...
        help=
        'The path to the sqlite3 database file. All the results will be saved there.'
    )
    parser.add_argument(
        '--search-model',
        action='store_true',
        help=
        'Whether to search the model. If not, use the models specified in the config.toml.'
    )
    parser.add_argument(
        '--manual-job-timeout',
        type=int,
        default='600',
        help=
        'The timeout threshold in seconds for jobs using manual stage. (Default: 10min)'
    )
    parser.add_argument(
        '--retry-failed',
        action='store_true',
    ...
    for cluster_spec in config.generate_cluster_specs():
        for model_spec, training_spec in config.generate_models(cluster_spec):
            for parallel_spec in config.generate_parallel_specs(cluster_spec):
                train_with_alpa(args, db, cluster_spec, model_spec,
                                training_spec, parallel_spec)
